...
export const artifactToAbi = (artifact: any) => {
...
export const getPlaceholder = (type: string, item?: AbiInput) => {
  ...
  if (isStruct) {
    ...
    const structTypesPlaceholders = components.map((c: AbiInput) => {
      const typePlaceholder = getPlaceholder(c.type, c).replace(
        "Example: ",
        ""
      );
...
export const encodeSignature = (sig: string) =>
  utils.hexDataSlice(utils.id(sig), 0, 4).slice(2);

const validators: { [x: string]: Validator } = {
  [AbiInputType.ADDRESS]: (value: string) =>
    utils.isAddress(value) ? null : VALIDATE_ERROS.NOT_ADDRESS,
  "address[]": (value: string) =>
    validateArray(value, validators[AbiInputType.ADDRESS] as Validator),
  ...
  "bool[]": (value: string) =>
    validateArray(value, validators[AbiInputType.BOOLEAN] as Validator),
...
const getTupleArguments = (sig: string) => {
  const argsRegExp = /([^,]+\(.+?\))|([^,]+)/g;
  let args =
    sig.replace(AbiInputType.TUPLE, "").slice(1, -1).match(argsRegExp) || [];
...
export const encode = (parameters: Parameters) => {
  ...
  pInputs.forEach((item, index) => {
    ...
    const validator: Validator = (validators[keyType] ||
      (() => "")) as Validator;
    types.push(type);
...
export const isStructInput = (input?: AbiInput) =>
  input ? (input.type || "").includes(AbiInputType.TUPLE) : false;
export const hasFixedLengthArrayInput = (input?: AbiInput) =>
  input ? (input.type || "").match(/\[[0-9]+\]/) : false;
